...
-︙التواصل يمكنك التواصل مع المطور من هنا @T_T_1T_T_T لشراء تمويل.
...
-︙الروابط فقط المقبوله.
    ...
    if call.data == 'newpromote':
        ...
        if len(d['promotions']) >0:
            ...
            return
        if d['coin'] < 500:
          return bot.edit_message_text(text='نقاطك لاتكفي، تحتاج <strong>(500)</strong> نقطة على الاقل ..', chat_id=chat_id, message_id=call.message.id, reply_markup=back)
